function _updateTotal() {
  ...
  $(".ticket-row").each(function () {
    ...
    if (amountElement.length > 0) {
      ...
      amount = dataPrice !== undefined ? (parseFloat(dataPrice) || 0) : (parseFloat(amountElement.text().replace(/[^0-9.]/g, '')) || 0);
    } else {
      var priceEl = $(this).find(".ticket-price");
      var dp = priceEl.attr('data-price');
      amount = dp !== undefined ? (parseFloat(dp) || 0) : (parseFloat(priceEl.text().replace(/[^0-9.]/g, '')) || 0);
    }
...
function doCheckout() {
  ...
  if (checkoutBtn) { checkoutBtn.innerHTML = 'Processing...'; checkoutBtn.disabled = true; }
  ...
  fetch("/cart/checkout/", {
    ...
    .then(function (result) {
      ...
      if (!result.ok) return result.json().then(function (err) { return Promise.reject(err); });
      return result.json();
    })
    .then(function (data) {
